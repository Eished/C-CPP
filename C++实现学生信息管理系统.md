...
void School::GetInfo()
{
	...
	{
		Student s1;
		cin >> s1.m_ID >> s1.m_name;

		/*将学生存入数组，同时学生数量+1 */
		
		stuList[stuNumber++] = s1;
	}
}

/*
 *功能:按ID从小到大对数组中的学生信息进行排序
 *参数:
 *
 */
void School::Sort()
{
	for (int i = 0; i < stuNumber-1; i++)
	{
		...
		{
			if (stuList[j].m_ID < stuList[minIndex].m_ID)
			{
				minIndex = j;
			}
		}
		...
		if (i != minIndex)
		{
			Student s1;
			s1 = stuList[i];
			stuList[i] = stuList[minIndex];
			stuList[minIndex] = s1;
		}
		if(i < 2 || i == stuNumber - 2)
			this->PrintList();
...
int main(void)
{
	...
	sc1.GetInfo();
	sc1.Sort();
	for (int i = 0; i < sc1.stuNumber; i++)
	{
		cout << stuList[i].m_name << " ";
	}
	cout << endl;
